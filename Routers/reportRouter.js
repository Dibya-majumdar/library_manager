...
router.get("/issued-books", verifyToken, adminOnly, async (req, res) => {
  const transactions = await Transaction.find({ actualReturnDate: null }).populate("bookId");
  res.json(transactions);
});

router.get("/returned-books", verifyToken, adminOnly, async (req, res) => {
  const transactions = await Transaction.find({ actualReturnDate: { $ne: null } }).populate("bookId");
  res.json(transactions);
});

router.get("/fines", verifyToken, adminOnly, async (req, res) => {
